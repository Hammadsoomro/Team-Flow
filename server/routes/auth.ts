...
// Helper: Consistent JSON serialization for token payloads
const stringifyPayload = (payload: {
  ...
  // Use fixed key order to ensure consistent JSON output
...
const createToken = (user: User): string => {
  ...
  const signature = crypto
    .createHash("sha256")
    .update(payloadString + jwtSecret)
    .digest("hex");
...
export const verifyToken = (
  ...
  try {
    ...
    if (!encodedPayload || !signature) {
      console.error("Token format invalid - missing parts");
      return null;
    }
    ...
    if (payload.exp < Date.now()) {
      console.error("Token expired:", {
        exp: payload.exp,
        now: Date.now(),
        diff: payload.exp - Date.now(),
      });
      return null;
    }

    // Verify signature - use the same format as when creating the token
    const jwtSecret = process.env.JWT_SECRET || "demo-secret";
    const expectedSignature = crypto
      .createHash("sha256")
      .update(stringifyPayload(payload) + jwtSecret)
      .digest("hex");
